...
W2.NEW_ATTRS = [
 ...
 {id:'hollowhymn',  si:34,world:7, icon:'🎺',name:'Hollow Hymn',  tier:'MYTHIC',
  ...
  desc:'Lifesteal 8% · 60% status resistance · +30% healing',
  fx:{lifesteal:.08,dotRes:.60,healMult:1.30}},
...
W2.REACTIVE = {
  ...
  thepen  :{world:11,on:'streakBreak',  msg:'THE PEN forgives the broken streak.',oncePerRun:true}
};
...
W2.reactiveList = function(){
  const w=W2.curWorld();
  return Object.keys(W2.REACTIVE).filter(t=>w>=W2.REACTIVE[t].world);
};
...
W2.reactiveTry = function(event){
  try{
    ...
    const st=W2.reactiveState(); if(!st) return false;
    if(st.firedThisBattle.length>=W2.REACTIVE_CAP) return false;
...
W2.applyNewPU = function(type){
  ...
  switch(type){
    ...
    case 'restore': { const back=g.hpPenalty||0; g.hpPenalty=0;
                    ...
                    return true; }
    case 'thepen':  say('✒️ THE PEN is held in reserve. It acts on its own.','#fff'); return true;
  }
  return false;
};
...
W2.onPageBroken = function(reason){
  if((G.inv&&G.inv.thepen>0) && G._pages>=W2.penThreshold()){
    G.inv.thepen--; inv();
