...
const BOSS_ATTRS=[
  ...
  /* Reflect used to fire only on a swing that connected, which made it the
     one damage source that punished you for defending well. reflectDot makes
     the burn and poison ticking on you chip back too, so the attribute has a
     floor even in a fight you are winning cleanly. */
  {id:'astralbarrier',  si:14, world:3, icon:'🌌', name:'Astral Barrier',   tier:'EPIC',
   desc:'Reflect 22% of damage taken — including damage over time · 8% dodge',
   fx:{thorns:.22, dodge:.08, reflectDot:true}},
  ...
  /* Was 15% DR and a second, smaller copy of Astral Barrier's reflect. hitCap
     gives it an axis nothing else has: a ceiling on the SINGLE worst hit. The
     defaults are 26/32/42% of your bar by tier, so a boss crit can take
     two-fifths of it in one blow — this caps every hit at 22% whatever the
     tier, which is worth most exactly where the game is most swingy. */
  {id:'ironcreator',    si:20, world:5, icon:'🛠️', name:'Iron Creator',     tier:'LEGEND',
   desc:'Take 15% less damage · no single hit can take more than 32% of your HP',
   /* Tuned twice by measurement. .22 won all 26 margin fights outright, which
      is the same failure as never winning one; .28 still won 25. .32 is the
      mini cap exactly, so it blunts only boss-tier blows — 42% of your bar in
      one hit becomes 32% — and does nothing at all on an ordinary level. */
   fx:{dr:.15, thorns:.06, hitCap:.32}},
  /* The clock half of this is the best attribute in the game at stage 50 and
     worth nothing in the margin, where pATK is seven figures and two seconds
     is 3.6 damage. noMomentum gives it a second half that matters in every
     long fight: the enemy's MOMENTUM (+10% per odd connected hit, to +60%)
     never builds. A loop does not escalate. */
  {id:'infiniteloop',   si:21, world:5, icon:'⏳', name:'Infinite Loop',    tier:'LEGEND',
   desc:'Immune to Paralyze & Freeze · +2s answer time · the enemy never builds MOMENTUM · its first blow of every battle misses',
   /* noMomentum alone left it bottom of all 26 fights: in the margin pATK is
      seven figures and every winning attribute is atkMult, lifesteal or
      regen, so a purely defensive one has to be worth a whole hit to compete.
      A free opening blow is that, it scales with the fight rather than with a
      number, and a loop starting over is the same idea as the rest of it. */
   fx:{immune:['paralyze','freeze','slowed'], timeBonus:2, noMomentum:true, freeFirstHit:true}},
  ...
  /* Pure resistance is invisible whenever the enemy is not using status, so
     it was a counter and nothing else. cleanAtk makes the resistance pay for
     itself: staying unafflicted is now a damage buff, and the 80% is what
     keeps you unafflicted. The attribute rewards its own stat. */
  {id:'celestialshield',si:23, world:5, icon:'💫', name:'Celestial Shield', tier:'MYTHIC',
   desc:'80% status resistance · take 10% less damage · +20% ATK while unafflicted',
   fx:{dotRes:.80, dr:.10, cleanAtk:.20}},
