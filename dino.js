...
/* ── combat resolution ── */
function attrFX(){
  /* cleanAtk / hitCap / noMomentum / reflectDot / overflow are the five axes
     added so the never-picked attributes have a reason to exist that is
     neither a counter nor a bigger number — see dino-world.js BOSS_ATTRS. */
  const f={dodge:0,dr:0,thorns:0,lifesteal:0,regen:0,dotRes:0,timeBonus:0,atkMult:1,healMult:1,
           cleanAtk:0,hitCap:1,noMomentum:false,reflectDot:false,overflow:false,
           freeFirstHit:false,immune:{}};
  (G.attrsEquipped||[]).forEach(id=>{
    ...
    f.cleanAtk+=x.cleanAtk||0;
    if(x.hitCap)f.hitCap=Math.min(f.hitCap,x.hitCap);   // the tightest cap wins
    if(x.noMomentum)f.noMomentum=true;
    if(x.freeFirstHit)f.freeFirstHit=true;
    if(x.reflectDot)f.reflectDot=true;
    if(x.overflow)f.overflow=true;
  ...
  f.cleanAtk=Math.min(.30,f.cleanAtk);
  f.hitCap=Math.max(.15,f.hitCap);      // never below the floor, however stacked
  return f;
}
...
const PU={
  ...
  freeze:  {icon:'❄', name:'FREEZE',  color:'var(--blue)',  border:'#4a9eff',desc:'Enemy skips 2 turns · stacks to 6'},
  ...
  paralyze:{icon:'⚡',name:'PARALYZE',color:'var(--cyan)',  border:'#00e5ff',desc:'Enemy skips 2 turns · stacks to 6'},
  ...
  rage:    {icon:'😤',name:'RAGE',    color:'var(--red)',   border:'#e84545',desc:'ATK×2 for 3 turns · stacks to 9 turns (not ×4) · costs 1% max HP/turn'},
  ...
  barrier: {icon:'🧱',name:'BARRIER', color:'var(--blue)',  border:'#4a9eff',desc:'Block the next 2 hits fully · stacks to 6 hits'},
...
/* ── §7b · THE CANON · where each certainty came from ─────────────────────
   Nothing in this world is held up by matter; it is held up by assertion. A
   correct answer re-states a piece of what has been established, and the world
   briefly does as it is told — which is why damage is built out of the
   question's tier, the clock and the streak, and why pATK is a rounding error
   for three whole worlds.

   A power-up is a fact asserted so heavily, for so long, that it stopped
   needing a mind to hold it and condensed into something you can carry. That
   is the whole mythology, and three real mechanics fall out of it:

     · rollDrop only ever rolls out of your own loadout — you only find the
       certainties you are already carrying, because you only notice what you
       already believe.
     · Worlds 1-5 have no spend budget and the margin's tightens every world —
       near the centre the Bank is dense and refills; past the margin the world
       is thin and there is less to draw on.
     · Heals never charge a use — restoring yourself is not drawn from the
       world, it is drawn from you.

   Boss stages hand out ATTRIBUTES, ordinary stages hand out certainties. The
   distinction is not cosmetic: an attribute is a certainty that has become
   part of you and is worn, a power-up is one you carry and spend.

   Written 19 Sep. Canon lives in claude/powerup-lore.md; keep the two in step.
   Display: the power-up guide modal only. The inventory tooltip and the pick
   cards stay numeric — mid-fight a player needs the figure, not the poem. */
/* ── §7a2 · RANK · how rare a certainty is ─────────────────────────────────
   Same ladder the attributes already use (RARE > EPIC > LEGEND > MYTHIC >
   ETERNAL), so the two systems read as one. ETERNAL was reserved for THE
   UNWRITTEN alone; AUTO-REVIVE now shares it, because it is the only power-up
   that undoes the one thing the game cannot otherwise undo.

   Rank is not decoration: RANK_WEIGHT is what GAMBLE rolls against, so a
   MYTHIC lands about a fifth as often as a RARE and the single ETERNAL is a
   jackpot rather than 1-in-22. Ranks follow measured share-of-a-boss-fight
   (see the run guides), not vibes. */
const PU_RANK={
  revive:'ETERNAL',
  halfhp:'MYTHIC', barrier:'MYTHIC', divine:'MYTHIC', thepen:'MYTHIC', restore:'MYTHIC',
  overload:'LEGEND', poison:'LEGEND', leech:'LEGEND', double:'LEGEND', desalt:'LEGEND',
  freeze:'EPIC', paralyze:'EPIC', nuke:'EPIC', burn:'EPIC', rage:'EPIC', stun:'EPIC',
  mirror:'EPIC', shield:'EPIC', loophole:'EPIC', anchor:'EPIC', echo:'EPIC',
  heal:'RARE', regen:'RARE', oracle:'RARE', fiftyf:'RARE', insight:'RARE', gamble:'RARE',
};
const RANK_WEIGHT={RARE:100, EPIC:70, LEGEND:40, MYTHIC:18, ETERNAL:12};
function puRank(t){ return PU_RANK[t]||'RARE'; }
function puWeight(t){ return RANK_WEIGHT[puRank(t)]||100; }
/* one weighted draw out of `pool` */
function weightedPick(pool){
  let total=0; for(const t of pool) total+=puWeight(t);
  if(total<=0) return pool[Math.floor(Math.random()*pool.length)];
  let r=Math.random()*total;
  for(const t of pool){ r-=puWeight(t); if(r<=0) return t; }
  return pool[pool.length-1];
}

const PU_LORE={
  // ── WORLD 1 · the elements, and the first things anything agreed on ──
  heal:    'Forest Fringe. The first thing living things agreed on: that a wound is temporary.',
  regen:   'Forest Fringe. Not the wound closing — the season turning. Slower, and it does not stop.',
  shield:  'Desert Dunes. Sand does not refuse a blow. It agrees to take most of it, three times over.',
  barrier: 'Coastal Cliffs. The sea has argued with this rock for an age, and the rock has not moved.',
  burn:    'Volcanic Valley. Fire is the certainty that nothing knits itself together while it is still burning.',

  // ── WORLD 2 · cold, venom, and the two things the Wastes refuse to respect ──
  freeze:  'Frost Peaks. Cold does not stop a creature. It stops the creature’s next two intentions.',
  poison:  'Poison Swamp. A patient fraction, taken from what a thing is rather than from what is left of it.',
  stun:    'Chaos Wastes. The Wastes keep no rule, so nothing there may claim exemption from this one.',
  gamble:  'Chaos Wastes. You do not choose. Something that does not care what you wanted chooses, and it is never nothing.',
  halfhp:  'Omega Citadel. Halving was the Citadel’s own idea — which is why everything built after it knows the trick.',

  // ── WORLD 3 · the Reactor’s three draws, and the fire that eats the holder ──
  double:  'Aether Reactor. The rated draw — exactly what the Reactor was built to give, given without complaint.',
  overload:'Aether Reactor. The same draw with the rating ignored. Some of it comes back through you on the way out.',
  paralyze:'Aether Reactor. Not damage. Instruction. The body is told to wait, and it waits.',
  rage:    'Never-Hell. It burns the one holding it. That is not a side effect — that is the fuel.',
  leech:   'Abyssal Trench. The deep takes a share of everything that passes, and the taking is also a feeding.',

  // ── WORLD 4 · the Forge, the Graveyard, and the shield that reads ──
  nuke:    'Starlight Forge. A star’s force is its mass. So is yours, and this is the certainty that says so out loud.',
  revive:  'Nebula Graveyard. Carry it and the Graveyard watches you more closely; fall far enough and it returns one whether you carried it or not.',
  mirror:  'Quantum Matrix. The shield reads the blow, agrees with it completely, and returns it word for word.',

  // ── WORLD 5 · the only certainty the gods kept for themselves ──
  divine:  'The astral Pantheon. Not healing — restoration, the difference being that restoration does not negotiate.',

  // ── THE ARCHIVE · the three that act on the question, not on the fight ──
  oracle:  'The Archive. It cannot make you right. It can show you which of the four was already true.',
  fiftyf:  'The Archive. It will not answer for you, but it will stop lying to you quite so freely.',
  insight: 'The Archive. Thirty seconds — the Archive’s entire opinion on what knowing is worth.',
};
...
/* ── one auto-grant, for every source that hands you a power-up ──────────
   There were two systems doing this and they disagreed. pickReward's mid
   branch had the good one — stack it if you carry it, queue a healing type
   as a GUARDIAN so it is not wasted at full HP, otherwise fire it and hold
   it pending until it lands. GAMBLE had its own: assign a charge, assign
   zero 350ms later, and hope. That second one destroyed stacks the player
   already owned and threw the roll away if the level ended inside the
   window.

   Both call this now, so "the game hands you a power-up" means one thing. */
const AUTO_HEALS=['heal','regen','divine'];
function autoGrant(type,opts){
  if(!PU[type])return 'unknown';
  opts=opts||{};
  /* already in the rail: it is a top-up, nothing to fire */
  if(!opts.neverStack && G.loadout && G.loadout.includes(type)){
    G.inv[type]=(G.inv[type]||0)+1;
    renderInventory();
    return 'stacked';
  }
  /* a heal you do not carry is worth more later than now */
  if(!opts.neverQueue && AUTO_HEALS.includes(type)){
  ...
  /* Anything else fires itself. WHEN is the caller's business: GAMBLE is a
     use, so it passes fireNow and the roll lands in the fight you are in. A
     boss reward is granted on the award screen between levels, where firing
     it would spend it on an enemy that is already dead — so it only sets the
     pending type and the next battle's tryAutoUse() picks it up. */
  G._autoUsePending=type;
  if(opts.fireNow)tryAutoUse();
  return G._autoUsePending===type ? 'pending' : 'fired';
}
...
function onAnswer(chosen,btnEl,idx){
  ...
  if(correct){
    ...
    const _af=attrFX();
    dmg*=_af.atkMult;
    /* CELESTIAL SHIELD: the resistance pays for itself. Anything the enemy has
       managed to put on you switches the bonus off, so the 80% status
       resistance on the same attribute is what keeps it switched on. */
    if(_af.cleanAtk>0){
      const afflicted=(G.activeEffects||[]).some(e=>/^player/.test(e.type)||e.type==='blindness');
      if(!afflicted)dmg*=(1+_af.cleanAtk);
    }
...
if(G._session !== _battleSession){ G.animLock=false; return; }
  ...
  G.bossAtkCounter++;
  /* INFINITE LOOP: the first blow of every battle lands on nobody. */
  if(G.bossAtkCounter===1 && attrFX().freeFirstHit){
    spawnFloat('LOOP','var(--cyan)',false);
    setMsg('⏳ INFINITE LOOP — the opening blow never happened.','var(--cyan)');
...
const bSup=burnSuppressFor(tier);
  ...
  let momentum=0;
  /* INFINITE LOOP: a loop does not escalate. */
  if(G.bossAtkCounter%2===1 && !attrFX().noMomentum){
  ...
  /* IRON CREATOR tightens the per-hit ceiling below whatever the tier allows
     — 42% of your bar in one boss blow is the swingiest number in the game. */
  dmg=Math.min(dmg,Math.round(G.playerMaxHP*Math.min(HIT_CAP[tier],attrFX().hitCap)));
...
/* opts.noCut marks a heal that is defined as a FRACTION OF THE BAR rather
   than an amount — DIVINE's "restore to full". The elemental layer shaves a
   percentage off any amount it is handed, and a full-heal gap looks like an
   amount to it, so DIVINE quietly restored 55% of the gap on a poison stage
   while its own message said "full health!". Measured: short on 13 of 51
   stages, worst 77.4% of the bar on POISON SWAMP. */
function doHealAura(amt,label,opts){
  ...
  /* HOLLOW HYMN: three stats that all said "more healing", against a bar that
     stops at full — the +30% was discarded exactly when it mattered most.
     What the bar cannot hold is dealt to the enemy instead. */
  const _oh=(typeof attrFX==='function')?attrFX():null;
  if(_oh&&_oh.overflow){
    const spill=Math.max(0,Math.round(Math.max(0,amt||0)-gained));
    if(spill>0&&G&&G.inBattle&&G.bossHP>0){
      G.bossHP=Math.max(0,G.bossHP-spill);
      G.score+=spill;
      if(typeof spawnFloat==='function')spawnFloat(`-${spill}🎺`,'var(--pink)',true);
...
function tickEffects(){
  ...
  G.activeEffects=G.activeEffects.filter(e=>{
    ...
    if(e.type==='playerPoison'||e.type==='playerBurn'){
      ...
      /* ASTRAL BARRIER: reflect used to need the enemy to connect, which made
         it the one damage source that punished you for defending well. The
         burn and the poison chip back too now. */
      const _rf=attrFX();
      if(_rf.reflectDot&&_rf.thorns>0&&G.bossHP>0){
        const back=Math.max(1,Math.round(d*_rf.thorns));
        G.bossHP=Math.max(0,G.bossHP-back);
        G.score+=back;
        spawnFloat(`-${back}🌌`,'var(--purple)',true);
      }
    }
...
const STACK_CFG={
  ...
  barrier:{turnsAdd:2,cap:3,turnsStack:true,turnsMax:6},   // 'stacks to 6' = 6 blocked hits
  freeze:{turnsAdd:2,cap:3,turnsStack:true,turnsMax:6},
  paralyze:{turnsAdd:2,cap:3,turnsStack:true,turnsMax:6},
  ...
  playerRage:{turnsAdd:3,cap:3,turnsStack:true,turnsMax:9},
...
function addOrStackEffect(type,extra){
  ...
  /* Two kinds of stacking effect, and one of them was silently doing nothing.
     An effect with a magKey (shield, poison, regen) scales its MAGNITUDE with
     the stack, so refreshing the timer is right. An effect whose whole value
     is its DURATION — the turns are what get consumed — gains nothing from a
     bigger `stack` unless the turns actually accumulate, and Math.max only
     refreshes them. Measured: rage, barrier, freeze and paralyze all reported
     "x2" and "x3" while every number stayed put, so the second charge was
     spent for nothing.  turnsStack opts an effect into real accumulation,
     bounded by turnsMax. All four are now opted in. BARRIER additionally had
     the wrong cap: 6 stacks x 2 turns is twelve blocked hits, against a
     tooltip promising six, so its cap dropped to 3 and turnsMax pins the
     ceiling at the advertised 6. */
  e.turns=cfg.turnsStack
    ? Math.min(cfg.turnsMax||Infinity,(e.turns||0)+turns)
    : Math.max(e.turns,turns);
  if(cfg.magKey){
...
function usePowerup(type){
  ...
  if(type==='divine'){
    G.inv[type]--;
    const healed=doHealAura(G.playerMaxHP-G.playerHP,'🌟 FULL HP',{noCut:true});
    sfx.powerup();
    /* say what actually happened: if something still held it back, the old
       message claimed full health at 77% of the bar */
    const full=G.playerHP>=G.playerMaxHP;
    setMsg(full?`🌟 DIVINE! Restored ${healed} HP — full health!`
               :`🌟 DIVINE! Restored ${healed} HP.`,'var(--yellow)');
    updateBars();renderInventory();return;
  }
    if(type==='gamble'){
      G.inv[type]--;
      /* REVIVE is no longer on the never-list. It is ETERNAL rank, so
         weightedPick draws it at roughly 1% — a jackpot rather than a
         1-in-22 — and the branch below GRANTS it instead of "using" it,
         because usePowerup('revive') only prints that it is armed. */
      const never=['gamble'].concat(((window.W2&&W2.NO_RANDOM)||[]).filter(t=>t!=='revive'));
      ...
      /* The one outcome that is not "used": a revive is held, not spent. */
      if(picked==='revive'){
        if(!reviveRoom()){                     // already netted — reroll once, revive barred
          const alt=pool.filter(t=>t!=='revive');
          if(!alt.length){G.inv[type]++;renderInventory();
            setMsg('Nothing to roll.','var(--grey)');return;}
          const p2=weightedPick(alt);
          sfx.powerup();setMsg(`GAMBLE! Auto-using ${PU[p2].name}`,'var(--pink)');
          spawnFloat(PU[p2].name,'var(--pink)',false);renderInventory();
          setTimeout(()=>{autoGrant(p2,{neverStack:true,fireNow:true});renderInventory();},350);
          return;
        }
        G.inv.revive=(G.inv.revive||0)+1;
        sfx.powerup();
        setMsg('🎲 ETERNAL! 🍖 AUTO-REVIVE secured.','var(--cyan)');
        spawnFloat('ETERNAL — REVIVE','var(--cyan)',false);
        renderInventory();
        if(typeof renderReviveSlot==='function')try{renderReviveSlot();}catch(e){}
        return;
      }
      ...
      renderInventory();
      /* GAMBLE now goes through autoGrant(), the same path a boss reward
         takes, so the game has ONE way of handing you a power-up instead of
         two that disagreed.

         What it replaced: `G.inv[picked]=1` and then, on a blind 350ms
         timer, `G.inv[picked]=0`. Measured, that ASSIGNS zero rather than
         spending one, so three things went wrong at once — rolling something
         you already carried destroyed the stack (five BURN became none), the
         rolled item sat in the rail as a real clickable charge for those
         350ms, and if the use was refused or the level ended inside the
         window the roll was zeroed anyway and simply vanished.

         neverStack: a GAMBLE is "use one", not "have one", so a roll you
         already carry still fires rather than quietly topping up the stack.
         The short delay stays — it is what lets the player read the
         "Auto-using X" line before the effect lands. */
      setTimeout(()=>{
        const how=autoGrant(picked,{neverStack:true,fireNow:true});
        if(how==='guardian')
          setMsg(`🎲 ${PU[picked].name} is held for when you need it.`,'var(--yellow)');
        else if(how==='pending')
          setMsg(`🎲 ${PU[picked].name} could not land — it fires next battle.`,'var(--grey)');
        renderInventory();
      },350);
      return;
  }
  ...
  if(type==='rage'){
    ...
    sfx.powerup();setMsg(`😤 RAGE ×${e.stack}! ATK×2 for ${e.turns} turns, -${rageCostFor()}HP/turn`,'var(--red)');
...
/* ── AUTO-REVIVE, three routes ─────────────────────────────────────────────
   Measured before rewriting: the single streak>=7 route put the median first
   revive at answer 42 on Medium at 90% accuracy (mean 57, unluckiest tenth
   119). Over a 600-900 answer run that is not rare — but 56 of 120 measured
   runs die in WORLD 1, 25 of them at stage 1, inside the first ~25 answers.
   The net was arriving after the danger.

   Worse, the gate ran backwards. streak>=7 awards the safety net for NOT
   struggling: median 93 answers at 80% accuracy against 22 at 100%. The
   players who needed it most were the least likely to be handed it.

   So the rate is kept and the SHAPE is fixed. Three routes that do not
   overlap:

     1 · EARNED     streak, as before — but carrying REVIVE in your loadout
                    lowers the gate and raises the roll instead of handing you
                    a free charge. The pick is an investment in the net, not
                    the net itself, and someone who never picks it still earns
                    one on the old terms.
     2 · THE GRAVEYARD  low HP, holding none, gate relaxed to 2. Scales as the
                    bar falls, so the net appears when you are actually in
                    trouble rather than when you are cruising. This is the one
                    that covers World 1.
     3 · GAMBLE     an ETERNAL-rank jackpot, ~1%. See the gamble branch.

   REVIVE is still filtered out of the ordinary drop pool by W2.dropPool(), so
   carrying it never dilutes your drop table — it only improves the net. */
const REVIVE_CHANCE=0.08;         // bare: of the answers where no other drop landed
const REVIVE_CHANCE_CARRIED=0.12; // carrying it is worth half again
const REVIVE_GATE=7;              // bare streak gate
const REVIVE_GATE_CARRIED=4;      // carrying it front-loads the net into World 1
const REVIVE_GATE_DANGER=2;       // the graveyard route barely gates at all
const REVIVE_DANGER_AT=0.35;      // "low HP" starts here
const REVIVE_DANGER_MAX=0.30;     // roll at 0 HP, scaling down to 0 at the line

/* 2 through the open game, 3 past the margin — where one boss hit is 42% of
   your bar and a single net does not cover a five-boss world. */
function reviveCap(){ return (typeof puWorldNow==='function' && puWorldNow()>=6) ? 3 : 2; }
const REVIVE_CAP=2;               // kept: older saves and tests read it
function reviveRoom(){ return ((G.inv&&G.inv.revive)||0)<reviveCap(); }
function reviveCarried(){ return !!(G.loadout&&G.loadout.includes('revive')); }

/* 0 when safe or already netted, rising to 1 as the bar empties */
function reviveDanger(){
  if(!G||!G.playerMaxHP)return 0;
  if(((G.inv&&G.inv.revive)||0)>0)return 0;      // only ever when you hold none
  const frac=G.playerHP/G.playerMaxHP;
  if(frac>=REVIVE_DANGER_AT)return 0;
  return (REVIVE_DANGER_AT-frac)/REVIVE_DANGER_AT;
}
...
function rollDrop(streak,diff){
  ...
  if(!reviveRoom())return null;
  const carried=reviveCarried();
  if(streak>=(carried?REVIVE_GATE_CARRIED:REVIVE_GATE)
     && Math.random()<(carried?REVIVE_CHANCE_CARRIED:REVIVE_CHANCE))return 'revive';
  const d=reviveDanger();
  if(d>0 && streak>=REVIVE_GATE_DANGER && Math.random()<d*REVIVE_DANGER_MAX)return 'revive';
  return null;
}
...
function confirmPickPU(){
  ...
  /* REVIVE obeys its cap here too. The cap used to live only in reviveRoom(),
     which rollDrop consults and this loop did not — so carrying revive from
     the loadout screen through every world pick banked 2,4,7,11,15,19,24,29,
     35,41 of them by World 11, against a stated cap of 2. Measured, not
     theorised. */
  if(refill)G.loadout.forEach(t=>{
    const n=(G.inv[t]||0)+refill;
    G.inv[t]= (t==='revive') ? Math.min(n,reviveCap()) : n;
  });
  saveGame();
...
function pickReward(type){
  if(!PU[type])return;
    ...
    if(G._pickMode==='mid'){
    /* The three branches that used to be written out here — stack it if you
       carry it, queue a healing type as a GUARDIAN rather than waste it at
       full HP, otherwise fire it and hold it pending until it lands — are
       autoGrant() now, because GAMBLE needed exactly the same three and was
       doing its own broken version of them. One system, two callers.

       Two wrong versions preceded this logic. The first handed over a
       ...
       the prize for beating a boss silently evaporated. The second kept the
       charge, which turned every boss reward into inventory that piles up. */
    autoGrant(type);
    sfx.powerup();
...
{ t:'2 · THE SHAPE OF A RUN', html:`
  ...
  <ul class="ig-step-list">
    ...
    <li><span class="ig-step-dot">●</span><span>Beating <b>ETERNUS PRIME</b> in World 5 completes the run with a <b style="color:var(--yellow)">+5000</b> completion bonus.</span></li>
  </ul>
`},
...
{ t:'5 · STREAK & COMBO', html:`
  <ul class="ig-step-list">
    ...
    <li><span class="ig-step-dot">●</span><span>Streak 3+ adds <b>+10%</b> to your powerup drop chance. Streak <b>7+</b> can leave an AUTO-REVIVE — <b>4+</b> if you carry one.</span></li>
...
{ t:'7 · HOW THE ENEMY HITS BACK', html:`
  <div class="ig-flow">
    ...
    <div class="ig-flow-arrow">▼</div>
    <div class="ig-flow-step"><div class="ig-flow-num">3</div><div class="ig-flow-text"><b>Crit</b> 10 / 14 / 18% by tier for ×1.6 — and separately <b>TRUE DMG</b> 5 / 8 / 12%, which ignores all your RESIST.</div></div>
    <div class="ig-flow-arrow">▼</div>
  ...
  <div class="ig-callout">🛡 Every hit is capped as a share of your max HP — 26% normal, 32% mini, 42% boss. Nothing can one-shot you from full. Iron Creator is the only attribute that lowers the ceiling itself, pulling boss hits down to 32%.</div>
`},
...
{ t:'13 · HOW YOU GET POWERUPS', html:`
  <ul class="ig-step-list">
    ...
    <li><span class="ig-step-dot">●</span><span><b>🍖 AUTO-REVIVE</b> is the exception: it is never in the drop table and never refills past its cap — see the next page.</span></li>
...
{ t:'14 · AUTO-REVIVE', html:`
  ...
  <ul class="ig-step-list">
    <li><span class="ig-step-dot">●</span><span>Its own slot above your powerups, and it <b>cannot be used by hand</b>. It spends itself the instant a hit would end the run, putting you back at <b>25% HP</b>.</span></li>
    <li><span class="ig-step-dot">●</span><span><b>1 · Carry it.</b> Taking it at a loadout screen hands you <i>no</i> charge — it improves your odds instead: the streak gate drops from <b>7</b> to <b style="color:var(--yellow)">4</b>, the roll rises from 8% to <b style="color:var(--yellow)">12%</b>. That halves the wait, ~20 answers instead of ~40.</span></li>
    <li><span class="ig-step-dot">●</span><span><b>2 · Earn it.</b> Carried or not, any answer that drops nothing else can leave a revive instead.</span></li>
    <li><span class="ig-step-dot">●</span><span><b>3 · Fall far enough.</b> Below <b style="color:var(--red)">35% HP</b> holding none, the odds climb as your bar empties and the gate relaxes to 2 — a reprieve, not a guarantee.</span></li>
    <li><span class="ig-step-dot">●</span><span><b>🎲 GAMBLE</b> can land one too — the <b style="color:var(--cyan)">rarest single outcome</b> it can roll, about <b>1 in 100</b>. You can hold <b>two</b>; the cap rises later on.</span></li>
  </ul>
  <div class="ig-callout">💡 Carrying it costs you no drops — AUTO-REVIVE is never in the drop table, so your other items keep their full share of every roll.</div>
`},
...
{ t:'16 · BOSS ATTRIBUTES', html:`
  <ul class="ig-step-list">
    ...
    <li><span class="ig-step-dot">●</span><span>You start with <b>1 slot</b>. A <b>2nd</b> opens at 15 stages cleared (World 3), a <b>3rd</b> at 25, and two more beyond that.</span></li>
    <li><span class="ig-step-dot">●</span><span>They give dodge, damage reduction, reflect, lifesteal, regen, status resistance, ATK% or healing% — plus, since the rework, a <b>ceiling on the worst single hit</b>, <b>ATK while unafflicted</b>, <b>overflow healing</b>, reflect that fires on DoT, <b>momentum suppression</b> and a <b>free opening blow</b>.</span></li>
  ...
  <div class="ig-callout">🔒 However you stack them: 30% for dodge, damage reduction, reflect and unafflicted-ATK; 40% lifesteal; 90% status resistance; 8% regen; and no hit ceiling below 15%. Swap between worlds, not between fights.</div>
`},
  ...
  if(sub)sub.innerHTML=`Powerups scale with your world — <b style="color:var(--purple)">TIER ${puTier()}</b>. `+
    ...
    const items=have.map(t=>{
      ...
      return `<div class="pug-item${hide?' pug-mystery':''}">
        ...
        <div class="pug-text">
          <div class="pug-name">${hide?MASK:d.name}${hide?'':` <span class="pug-rank r-${puRank(t).toLowerCase()}">${puRank(t)}</span>`}${(!hide&&w)?` <span class="pug-w">W${w}</span>`:''}${(!hide&&auto)?' <span class="pug-auto">AUTO</span>':''}</div>
          <div class="pug-desc">${hide?MASK:puDesc(t)}</div>
          ${(!hide&&PU_LORE[t])?`<div class="pug-lore">${escHTML(PU_LORE[t])}</div>`:''}
        </div>
      </div>`;
    }).join('');
...
/* apply(arguments), not (amt): this wrapper took one parameter and dropped
   the rest, so the '🌟 FULL HP' and '💚 HEAL' labels never reached the float
   — every zero-gain heal showed the generic "MAX HP" — and it would have
   swallowed the noCut flag too. Forwarding everything makes it immune to the
   next signature change as well. */
window.doHealAura=function(){
  const b=box('p'); AR.orbs(b.cx,b.cy+b.h*.35,14,'#4ecb71'); AR.ring(b.cx,b.cy,34,'#4ecb71',2);
  return _heal.apply(this,arguments);
};
...
(function(){
  ...
  if(typeof realHeal==='function'){
    window.doHealAura=function(amt,label,opts){
      const p=(window.G&&G.inBattle)?G._ec:null;
      if(p&&p.healCut>0&&amt>0&&!(opts&&opts.noCut)){
      ...
      return realHeal.call(this,amt,label,opts);
    };
  }
})();
